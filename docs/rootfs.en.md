...
Patches applied: 0001 through 0010 (networking fixes, halt/reboot argument support, display encoding, stime removal, etc.)
...
After the base image is created, optional packages are installed via `pkg/pkg.sh install-all`. Packages are defined in `pkg/available/` and controlled by `pkg/package-config`. See [Packages](packages.en.md) for details.
