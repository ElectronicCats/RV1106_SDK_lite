...
Después de crear la imagen base, los paquetes opcionales se instalan mediante `pkg/pkg.sh install-all`. Los paquetes se definen en `pkg/available/` y se controlan con `pkg/package-config`. Consulte [Paquetes](packages.es.md) para más detalles.
