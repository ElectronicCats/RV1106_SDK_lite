...
[`riscv2arm.md`](../../riscv2arm.md) (codemap of the Luckfox SDK).
...
2. **Which RX (A2B) registers can the SCR1 READ?** — `A2B_STATUS` no; what about
   `A2B_CMD(3)` / `A2B_DAT(3)`? If the MCU can read `A2B_DAT(3)`, the clean fix
...
- `0xff6ff908` = `0x04` and `0xff6ff90c` = `0x524D5347` → **the MCU CAN read
  A2B_CMD/DAT** → clean fix: poll `A2B_DAT(3)==magic` as kick detector,
