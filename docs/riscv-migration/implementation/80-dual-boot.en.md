...
## 3. The chain of five bugs (history and the why of each fix)

1. **Kernel at `0x8000` clobbered the MCU** (`zImage` with AUTO_ZRELADDR
   self-decompresses to a fixed `0x8000`). *Fix:* FIT with raw `Image` compressed
...
2. **TEXT_OFFSET**: `textofs-$(CONFIG_CPU_RV1106) := 0x208000` was inside
   ...
   *Fix:* the RV1106 line was moved out of the ifeq.
3. **`no-map` on `rtos@40000` wiped all RAM**: the NOMAP hole makes the
   ...
   `early_alloc` in `paging_init`. *Fix:* `rtos@40000` and `ramoops@7c000` are
...
5. **The MCU's SystemInit hangs initializing the DCACHE** (`0xff640000`,
   ...
   block). *Provisional fix:* `# CONFIG_RT_USING_CACHE is not set` in
   `board/pwncube/defconfig` — the MCU runs without cache, which also guarantees
   coherency of the IPC shared-memory (see doc 70). *Pending:* port the
