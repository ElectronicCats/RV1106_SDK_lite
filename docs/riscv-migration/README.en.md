...
| 80 | [`dual-boot`](implementation/80-dual-boot.md) | **Dual boot WORKING**: definitive memory map, the 5-bug chain and its fixes, correct flashing (UF, never `DI -b`), marker diagnostics. |
...
3. **SPI/I²C on the MCU are HAL-only, not wired by the BSP.** They are driven
   ...
   the bus is ceded from Linux in the device tree. *(Resolved in the implementation.)*
4. **The IPC stack existed but was NOT wired for the RV1106** (rpmsg driver
   match, porting, device tree). *(Resolved: rpmsg transport working via vring
