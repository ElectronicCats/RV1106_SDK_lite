...
[`riscv2arm.md`](../../riscv2arm.md) (codemap del SDK Luckfox).
...
2. **¿Qué registros de RX (A2B) puede LEER el SCR1?** — `A2B_STATUS` no; ¿y
   `A2B_CMD(3)` / `A2B_DAT(3)`? Si la MCU puede leer `A2B_DAT(3)`, el fix limpio
...
- `0xff6ff908` = `0x04` y `0xff6ff90c` = `0x524D5347` → **la MCU SÍ puede leer
  A2B_CMD/DAT** → fix limpio: sondear `A2B_DAT(3)==magic` como detector de kick,
