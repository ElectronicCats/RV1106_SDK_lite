...
Hardware and libraries are present, **but the concrete integration for the RV1106
is not done** (see §8). The real wiring tested in the SDK is for RK3568.
...
**The RV1106-specific IRQ numbers are not defined** in the porting files
(today they inherit the RK3568 ones) → to be confirmed against the RV1106 `soc.h` (§8).
...
## 8. What is NOT done for the RV1106 (wiring work)

To have a functional A7↔RISC-V channel in the CubeSat one has to **add**, not just
recompile:

1. **Kernel:** add `"rockchip,rv1106-rpmsg"` to the match table of
   `rockchip_rpmsg.c:402-406` (or confirm that `rk3568` is binary-compatible).
2. **Device tree (Linux):** `rpmsg` node linked to `mailbox@ff5c0000` + `reserved-memory`
   region for the vrings (64 KB/instance) — **it does not exist** in the RV1106 DTS.
3. **RPMsg-Lite porting (RISC-V):** create `platform/RV1106/` (or adapt the one from
   `RK3568`) with the correct IRQ bases and vring addresses of the RV1106.
4. **RV1106 IRQ:** set the real `MBOX0_CHn_{A2B,B2A}_IRQn` from the RV1106 `soc.h`
   (today the RK3568 ones are used).
