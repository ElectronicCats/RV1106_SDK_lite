...
official SDK its typical use is the image/ISP pipeline and *wake-on-motion*; for
the CubeSat we will reuse it as a deterministic hardware controller.
...
point, see doc 30). It is the same RISC-V toolchain that will need to be integrated into
...
| Luckfox element | Reuse in CubeSat |
|------------------|--------------------------|
| BSP `rv1106-mcu` + common HAL | **Base** of the RISC-V firmware. It starts from a reduced `board/` variant (without camera sensors). |
| Toolchain `riscv-none-embed-gcc 10.2.0` | Integrate into the CubeSat's `build.sh` (new `mcu` target). |
| `link.lds` (0x40000/240 KB) | Reuse as-is; watch footprint. |
| SPL `Hpmcu` flow / `spl_fit_standalone_release` | Reuse the loading mechanism; the CubeSat already uses rkbin (see `src/rkbin`, `docs/uboot.md`). |
