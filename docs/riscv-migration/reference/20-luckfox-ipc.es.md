...
> **Naturaleza del documento.** Estudio del **SDK Luckfox** como referencia técnica
> (Fase 8 del plan). Rutas relativas al SDK. Cumple la *instrucción obligatoria*
> del plan: localizar y documentar la comunicación A7↔RISC-V real, citando archivos.
...
Hardware y librerías están presentes, **pero la integración concreta para el RV1106
no está hecha** (ver §8). El cableado real probado en el SDK es para RK3568.
...
**Los números de IRQ específicos del RV1106 no están definidos** en los archivos de
porting (hoy heredan los de RK3568) → a confirmar contra `soc.h` del RV1106 (§8).
...
## 8. Lo que NO está hecho para el RV1106 (trabajo de cableado)

Para tener un canal A7↔RISC-V funcional en el CubeSat hay que **añadir**, no solo
recompilar:

1. **Kernel:** añadir `"rockchip,rv1106-rpmsg"` a la tabla de match de
   `rockchip_rpmsg.c:402-406` (o confirmar que `rk3568` es compatible binario).
2. **Device tree (Linux):** nodo `rpmsg` enlazado a `mailbox@ff5c0000` + región
   `reserved-memory` para los vrings (64 KB/instancia) — **no existe** en los DTS del RV1106.
3. **Porting RPMsg-Lite (RISC-V):** crear `platform/RV1106/` (o adaptar el de
   `RK3568`) con las bases de IRQ y direcciones de vring correctas del RV1106.
4. **IRQ del RV1106:** fijar los `MBOX0_CHn_{A2B,B2A}_IRQn` reales desde el `soc.h`
   del RV1106 (hoy se usan los de RK3568).
5. **Dirección física de vrings:** definirla coherentemente en ambos lados (DT
   reserved-memory ↔ porting del MCU).

Estos cinco puntos son el contenido de la **Fase 10, paso 0** del roadmap (doc 50):
"levantar el transporte IPC" antes de migrar cualquier driver.
