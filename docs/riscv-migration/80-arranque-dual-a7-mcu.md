...
## 3. La cadena de cinco bugs (histórico y por qué de cada fix)

1. **Kernel en `0x8000` pisaba al MCU** (`zImage` con AUTO_ZRELADDR se
   auto-descomprime a `0x8000` fijo). *Fix:* FIT con `Image` cruda comprimida
...
2. **TEXT_OFFSET**: `textofs-$(CONFIG_CPU_RV1106) := 0x208000` estaba dentro de
   ...
   *Fix:* la línea del RV1106 quedó fuera del ifeq.
3. **`no-map` en `rtos@40000` borraba toda la RAM**: el hueco NOMAP hace que el
   ...
   `early_alloc` en `paging_init`. *Fix:* `rtos@40000` y `ramoops@7c000` son
...
5. **SystemInit del MCU se cuelga inicializando el DCACHE** (`0xff640000`,
   ...
   bloque). *Fix provisional:* `# CONFIG_RT_USING_CACHE is not set` en
   `board/pwncube/defconfig` — el MCU corre sin caché, lo que además garantiza
   coherencia del shared-memory de IPC (ver doc 70). *Pendiente:* portar la
