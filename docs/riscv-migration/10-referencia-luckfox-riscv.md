...
> **Naturaleza del documento.** Estudio del **SDK Luckfox** (`/home/heikki/Documents/luckfox-pico`)
> como referencia técnica. Todas las rutas son relativas a ese SDK. No describe
> este repo CubeSat. Cubre las Fases 2 y 4 del plan.
...
SDK oficial su uso típico es el pipeline de imagen/ISP y el *wake-on-motion*; para
el CubeSat lo reutilizaremos como controlador determinista de hardware.
...
fijo, ver doc 30). Es el mismo toolchain RISC-V que habrá que integrar en el
...
| Elemento Luckfox | Reutilización en CubeSat |
|------------------|--------------------------|
| BSP `rv1106-mcu` + HAL común | **Base** del firmware RISC-V. Se parte de una variante de `board/` reducida (sin sensores de cámara). |
| Toolchain `riscv-none-embed-gcc 10.2.0` | Integrar en el `build.sh` del CubeSat (nuevo target `mcu`). |
| `link.lds` (0x40000/240 KB) | Reutilizar tal cual; vigilar footprint. |
| Flujo SPL `Hpmcu` / `spl_fit_standalone_release` | Reutilizar el mecanismo de carga; el CubeSat ya usa rkbin (ver `src/rkbin`, `docs/uboot.md`). |
| `start_rv1106_mcu.S`, `HAL_RISCVIC_IRQHandler` | Sin cambios (arranque e IRQs del núcleo). |

**Pendiente de confirmar (riesgos):** (a) qué *board variant* mínima usar como punto
de partida; (b) que el flujo rkbin del CubeSat acepte el `LOADER2=Hpmcu` (hoy el
CubeSat no empaqueta firmware de MCU); (c) presupuesto real de RAM tras enlazar
RT-Thread + un driver. Estos puntos se resuelven en la Fase 10 (doc 50).
