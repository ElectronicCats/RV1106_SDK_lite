...
- **Linux glue (to be rewritten):** SPI/I²C access (spi core / regmap), GPIO
...
| `../sx1262-cli/sx1262_cli.c` (1–340) | CLI: opens `/dev/sx1262-{0,1}`, dispatches via ioctl/read/write. **[relocate as IPC client]** |
...
**Glue to be rewritten on top of the MCU HAL:** SPI (`spi_sync` → `HAL_SPI_*`/`drv_spi`),
GPIO (`gpiod_*` → `HAL_GPIO_*`/`rt_pin_*`), DIO1 IRQ (`request_irq` → `rt_pin_attach_irq`
or BUSY polling), and the external interface (char device/ioctl → IPC commands; doc 50).
...
Nodes: `rv1106-sdk-ipc.dtsi` ~`:256-273` (#0), ~`:275-292` (#1). Defaults: 868 MHz,
LoRa SF7/BW125/CR4-5, 14 dBm, RX at boot, IRQ TX/RX_DONE+TIMEOUT→DIO1.

> **Migration blocker (see doc 10 §3, doc 40):** SPI is not enabled on the
> MCU (`RT_USING_SPI` off) and SPI0 is owned by Linux in the DT. Migrating the SX1262 requires
> enabling SPI in RT-Thread and reassigning the SPI controller(s) to the RISC-V.
...
**Glue:** i2c_driver/probe → HAL I²C; regmap → direct access; IIO/sysfs →
export via IPC (doc 50).

> **Favorable to migration:** the MCU **does** have I²C (HAL + `drv_i2c`, with
> I2C4 enabled by default; Luckfox MCU doc). I²C0 (the CubeSat pins) must be routed to the
> MCU and the bus arbitrated if Linux also uses it.
...
**Glue:** i2c_driver/regmap/IIO/sysfs → HAL I²C + export via IPC; AD0 gpio-hog
→ fix the pin from the MCU firmware (or keep it in the Linux DT before
boot).
...
| Driver | Bus | Portable logic | Linux glue | Enabled on MCU today |
|--------|-----|-----------------|------------|------------------------|
| **SX1262 ×2** | SPI 8 MHz | SPI frames, init, calibration, radio encoding, IRQ parsing | spi core, gpiod, IRQ, chardev/ioctl, DT | **SPI: NO** (HAL only; SPI0 owned by Linux) |
| **BME280** | I²C0 400 kHz | Calib + Bosch compensation, register map | i2c, regmap, IIO/sysfs, DT | **I²C: YES** (HAL+drv_i2c) |
| **ICM-42670** | I²C0 400 kHz | Reset/config, BE reading, fixed-point scaling | i2c, regmap, IIO/sysfs, gpio-hog, IRQ | **I²C: YES** |
...
- SX1262 DIO2/DIO3 are unused (external switch + hardwired XTAL) — confirmed in
  code; keep in the MCU version.
- ICM ODR 100 Hz encoding (0x09 vs 0x08 per the datasheet table).
- Whether I²C0 (the CubeSat pins) can be assigned to the MCU without breaking Linux (bus
  arbitration / exclusive ownership).
