...
The SDK derives from the **Luckfox Pico SDK**, but it is **self-contained**:
both toolchains (ARM and RISC-V) and all source are already versioned. After
cloning **nothing else is downloaded** — you don't need the network during the
build.
...
The download is large (toolchains + versioned sources). If you only want to
build and don't care about git history, do a shallow clone to pull less:
